...
def show_menu() -> str:
    ...
    |
    |   {BLUE}CMS{RESET}
    |   {GREEN}[4] {BOLD}Сканер плагинов WordPress: wp-plugins{RESET}
    |   {GREEN}[5] {BOLD}Определить версию WordPress: wp-version{RESET}
    """
...
def main():
    try:
        ...
        if len(params) == 0:
            ...
            elif "wp-plugins" in item or item == "4":
                """WP plugins"""
                scanWordPressPlugins(url=input("| URL: ").strip())
            
            elif "wp-version" in item or item == "5":
                """WP version"""
